...
# 363 OTUS in 130 tree samples 

# Pull out OTU table of raw counts from phyloseq object 
otus <- phyloseq::otu_table(ps_AM) %>% as.data.frame()
...
tax <- tax_table(ps_AM) %>% as.data.frame() %>% rownames_to_column(var = "OTU")
...
# OTU's are all in the Glomeraceae
...
# Soil C
hist(sample_metadata$pct_C)

# Soil Sand
hist(sample_metadata$Sand)

# Soil Cation Exchange Capacity 
hist(sample_metadata$EC)
...
extremes_sig_results$Environmental_Var <- factor(extremes_sig_results$Environmental_Var, 
                                                 levels=c("Zn", "Tot_Bases", "SPEI", "SO4", "Silt", "Sev_Dry", "Sand", "pH", "Na", 
                                                          "MSP", "Mod_Dry", "Mg",  "MAT", "MAP", "K", "Fe", "Elev", "EC", "Cu", "Ca", "B"))

levels(extremes_sig_results$Environmental_Var)



# This won't be visible in the dataframe but


# Add column for if the environmental variable is related to drought or soil 

extremes_sig_results$Var_Type <- c("Spatial", "Spatial", "Drought", "Drought", "Drought",
                                   "Drought", "Drought", "Drought", "Drought", "Drought",
                                   "Drought", "Drought", "Drought", "Soil", "Soil", "Soil",
