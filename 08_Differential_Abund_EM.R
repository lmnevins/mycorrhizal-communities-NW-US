# -----------------------------------------------------------------------------#
# Differential Abundance Analyses for EM Communities using ALDEx2
...
## Significant but giant 

# Filter to just comparisons that have a significant result 
all_results_host_site$Significant <- as.factor(all_results_host_site$Significant)

all_results_host_site_filtered <- all_results_host_site %>%
  group_by(Site, Comparison) %>%
  filter(any(Significant == "Significant")) %>%
  ungroup()

# Replot

host_site_sig_plot <- ggplot(all_results_host_site_filtered, aes(x = effect, y = -log10(we.eBH), color = Significant)) +
  geom_point(alpha = 0.7, size = 1.5) +
  scale_color_manual(values = c("Not Significant" = "grey70", "Significant" = "#D7263D")) +
  geom_vline(xintercept = c(-0.6, 0.6), linetype = "dashed", color = "black", linewidth = 0.4) +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "black", linewidth = 0.4) +
  facet_wrap(~ Facet_Label, scales = "free_y") +
  labs(x = "Effect Size", y = "-log10 (p-value)", color = "Significant") +
...
# Filter to just comparisons that have a significant result 
all_results_hosts_across$Significant <- as.factor(all_results_hosts_across$Significant)

all_results_hosts_across_filtered <- all_results_hosts_across %>%
  group_by(Host_ID, Site_Comparison) %>%
  filter(any(Significant == "Significant")) %>%
  ungroup()

# Replot

hosts_across_sig_plot <- ggplot(all_results_hosts_across_filtered, aes(x = effect, y = -log10(we.eBH), color = Significant)) +
  geom_point(alpha = 0.7, size = 1.5) +
  scale_color_manual(values = c("Not Significant" = "grey70", "Significant" = "#D7263D")) +
  geom_vline(xintercept = c(-0.6, 0.6), linetype = "dashed", color = "black", linewidth = 0.4) +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "black", linewidth = 0.4) +
...
write.csv(extreme_tree_groups, "~/Dropbox/WSU/Mycorrhizae_Project/Community_Analyses/FINAL/EM_extreme_tree_groups.csv")
...
# Save file 
write.csv(extremes_sig_results, "~/Dropbox/WSU/Mycorrhizae_Project/Community_Analyses/FINAL/EM_extremes_diff_abund.csv")

extremes_sig_results <- read.csv("~/Dropbox/WSU/Mycorrhizae_Project/Community_Analyses/FINAL/EM_extremes_diff_abund.csv")
